...
const OFFRES = [
  {
    id: "vip" as Plan,
    label: "VIP \u2014 Acc\u00e8s chapitres 4 \u00e0 10",
    price: VIP_PRICE,
    priceFuture: "4,99\u00a0\u20ac",
    isEarlyAccess,
    description: "Acc\u00e8s permanent aux chapitres 4 \u00e0 10 d\u00e8s leur sortie.",
    badge: isEarlyAccess ? "Early access" : "Disponible",
    highlight: isEarlyAccess,
  },
];

function PlanLabel({ plan }: { plan: Plan }) {
  if (plan === "gratuit") return <>Chapitres 1\u20113 \u00b7 Gratuit</>;
  if (plan === "vip") return <>VIP \u00b7 Chapitres 1\u201310</>;
  return <>S\u00e9rie compl\u00e8te</>;
}

export default function SubscriptionPanel({ plan, chaptersUnlocked }: SubscriptionPanelProps) {
  const [showConfirm, setShowConfirm] = useState(false);

  const isVipOwned = plan === "vip" || plan === "serie";
    ...
    <div className="flex flex-col gap-4">
      ...
      {OFFRES.map((offre) => (
        <button
          ...
          type="button"
          disabled={isVipOwned}
          onClick={() => !isVipOwned && setShowConfirm(true)}
          className={`w-full rounded-xl border p-4 text-left transition-all ${
            isVipOwned
              ? "border-amber-500/30 bg-amber-500/[0.06] opacity-70 cursor-default"
              : offre.highlight
              ? "border-amber-500/50 bg-amber-500/[0.06] hover:bg-amber-500/10 cursor-pointer"
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              ...
              {offre.isEarlyAccess && !isVipOwned && (
                <p className="mt-1.5 text-[11px] text-amber-600 dark:text-amber-400 font-medium">
                  Prix normal\u00a0: {offre.priceFuture} d\u00e8s le 15\u00a0octobre\u00a02026
                </p>
              )}
            </div>
